...
//    self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"myroom_base"]];
    ...
    // specialDay-content
    self.specialDay = [[LYButtonImageView alloc] initWithFrame:Rect(60, MinY(logo.frame) + 30, 97, 66)];
    ...
    //  相片框1
    UIImageView *photoBorder = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"myroom_photoframe.png"]];
    ...
    if (userArray.count == 0) {
        lxyUserTableModel *user = [[lxyUserTableModel alloc] initWithID:0 andName:nil andPWD:nil andBirthday:nil andHeaderImage:nil andIntroduce:nil andPhoto1:nil andPhoto2:nil];
...
//    UIImage *image2 = [[UIImage alloc] initWithContentsOfFile:imagePath2];
    ...
    self.day = [[UILabel alloc] initWithFrame:Rect(MinX(_date.frame), MaxY(_date.frame), 42, 40)];
...
//    self.day.backgroundColor = [UIColor yellowColor];
    ...
    //  日记本
    self.book = [[LYButtonImageView alloc] initWithImage:[UIImage imageNamed:@"myroom_diary.png"]];
    self.book.frame = Rect(MaxX(_day.frame) - 20, MaxY(_setting.frame) + 10, 95, 90);
    self.book.tag = 0;
    [self addSubview:_book];
    
    // 沙漏
    self.store = [[LYButtonImageView alloc] initWithImage:[UIImage imageNamed:@"hourglass"]];
